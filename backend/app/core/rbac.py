...
def require_permissions(required: List[str]) -> Callable:
    ...
    async def _guard(
        ...
        # First check if user was already set on request.state by get_current_user
        user = getattr(request.state, "user", None)

        # If not, authenticate from the Bearer token directly
        if user is None:
            if not credentials:
        ...
        if not tenant_id:
            # If no RBAC tables populated yet, allow owner/admin access
            return True
        ...
        try:
            ...
            # RBAC tables may not exist yet — allow access
            return True

        # If no RBAC roles assigned yet (fresh tenant), allow all
